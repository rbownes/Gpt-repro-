...
| 11-loopllm | Weight-tied loop (1 shared Block × n_layer=12 iters) on v0.3 MHA, u_net_skips forced off | **hard-rejected** | 3.406 (Δ **+0.442**) | 177 k (−0.65 %) | capacity shortfall dominates (45.7 M vs 123.7 M, −63 % params). Kill-early at step 2000 triggered but overridden by experimenter; full 10 B run kept for loss-curve data. Gap plateaus at +0.44 (no closure). Looping gives ~0.14-nat boost over a size-matched untied baseline, nowhere near enough to cover the 63 % param cut. See report for exp/12 replanning. |
